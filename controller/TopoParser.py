...
from dst_modified_dijkstra import single_source_dijkstra_ecmp
...
class TopoParser:
    ...
    def calECMP(self):
        ...
        for source in range(self.num_nodes):
            for target in range(self.num_nodes):
                if source == target:
                    continue
                self.cdtpaths[source][target] = single_source_dijkstra_ecmp(self.graph,
                                            source, target)
        pathgenerator = nx.all_pairs_all_shortest_paths(self.graph, method="dijkstra")
        self.cdtpaths = dict(pathgenerator)
        self._reshapeCdtPaths()
        return self.cdtpaths
    ...
    def changeEdgeWeight(self, ratio):
        edges = list(self.graph.edges)
        num_to_change = int(round(len(edges) * ratio))
        edges_to_change = random.sample(edges, num_to_change)
        
        for source, target in edges_to_change:
            original_weight = self.graph[source][target]['weight']
            new_weight = int(round(original_weight * random.uniform(0.8, 1.2)))
    ...
    def getDeps(self):
        ...
        self.changeEdgeWeight(0.5)
        oldcdtpaths = self.cdtpaths
        # choose one
        self.cdtpaths = self.calECMP()
        # self.cdtpaths = self.calShortestPaths()
